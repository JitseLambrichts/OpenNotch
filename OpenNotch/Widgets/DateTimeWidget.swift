...
struct DateTimeWidget: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            Text(month)
                .font(appearance.font(size: 22, weight: .bold))
                ...
                ForEach(weekDates, id: \.self) { date in
                    ...
                    VStack(spacing: 4) {
                        Text(weekdayLetter(for: date))
                            .font(appearance.font(size: 10, weight: .semibold, design: .rounded))
                            .foregroundStyle(.white.opacity(isToday ? 0.95 : 0.45))

                        Text(String(format: "%02d", dayNum))
                            .font(appearance.font(size: 13, weight: isToday ? .bold : .medium, design: .rounded))
